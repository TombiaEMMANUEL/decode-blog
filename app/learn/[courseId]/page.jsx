    ...
    >
      Delete Course
        ...
        <div style={{ backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px", padding: "40px", marginBottom: "32px" }}>
          ...
          <h1 style={{ fontSize: "clamp(28px, 4vw, 44px)", fontWeight: 800, color: "white", lineHeight: 1.2, letterSpacing: "-1px", marginBottom: "16px" }}>
            {course.title}
          </h1>
          <p style={{ color: "#9ca3af", fontSize: "16px", lineHeight: 1.7, marginBottom: "24px" }}>{course.description}</p>
          ...
          {lessons.length > 0 && (
            <Link
              href={`/learn/${courseId}/${lessons[0].id}`}
              style={{ display: "inline-flex", alignItems: "center", gap: "8px", backgroundColor: "#7c3aed", color: "white", padding: "12px 28px", borderRadius: "12px", fontWeight: 600, fontSize: "15px", textDecoration: "none", marginTop: "28px" }}
            >
              <FiZap />
              Start Learning
            </Link>
        ...
        <div>
          ...
          {lessons.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px", backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px" }}>
              <p style={{ color: "#6b7280", fontSize: "14px" }}>No lessons yet. Check back soon!</p>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {lessons.map((lesson, index) => (
                <Link key={lesson.id} href={`/learn/${courseId}/${lesson.id}`} style={{ textDecoration: "none" }}>
                  <div
                    style={{ display: "flex", alignItems: "center", gap: "16px", backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "12px", padding: "16px 20px", transition: "all 0.2s", cursor: "pointer" }}
                  ...
                  >
                    <div style={{ width: "32px", height: "32px", borderRadius: "50%", backgroundColor: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.2)", display: "flex", alignItems: "center", justifyContent: "center", color: "#a78bfa", fontSize: "13px", fontWeight: 700, flexShrink: 0 }}>
                      {index + 1}
                    </div>
                    <div style={{ flex: 1 }}>
                      <p style={{ color: "white", fontSize: "15px", fontWeight: 500 }}>{lesson.title}</p>
                      {lesson.duration && (
                        <p style={{ color: "#6b7280", fontSize: "12px", marginTop: "4px", display: "flex", alignItems: "center", gap: "4px" }}>
                          <FiClock style={{ fontSize: "11px" }} />
                          {lesson.duration} min
                        </p>
                      )}
                    </div>
                    <FiCheckCircle style={{ color: "rgba(255,255,255,0.1)", fontSize: "18px" }} />
                  </div>
                </Link>
              ))}
            </div>
